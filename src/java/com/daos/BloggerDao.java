...
public class BloggerDao {
    ...
    public boolean checkUserStatus(Blogger blogger)
    {
        boolean status = false;
        try{
        Connection con = DbConnection.getConnection();
        if(con!=null)
        {
            String sql =" select * from blogger where userid=? and approvedStatus=\"pending\" or approvedStatus=\"denied\" ";
            PreparedStatement smt = con.prepareStatement(sql);
            smt.setString(1, blogger.getUserid());
        ...
        {
            System.out.println("checkuserstatus error :" + e.getMessage());
        }
        return status;
    }

     public boolean checkUser(Blogger blogger)
    {
        boolean status = false;
        try{
        Connection con = DbConnection.getConnection();
        if(con!=null)
        {
            ...
            PreparedStatement smt = con.prepareStatement(sql);
            smt.setString(1, blogger.getUserid());
            smt.setString(2, blogger.getPassword());
    ...
    public boolean setStatus(String statusValue, String userid){
    boolean status= false;
    Connection con = DbConnection.getConnection();
        try {
           
            String sql = "update blogger set approvedStatus=? where userid=?";
            PreparedStatement smt = con.prepareStatement(sql);
        ...
        {
            System.out.println("Blogger status Updation error :" + e.getMessage());
...
public int getIdByUserid(String userid)
{
    int id = 0;
        ...
        Connection con = DbConnection.getConnection();
        try {
            String sql = "select id from blogger where userid=?";
            PreparedStatement smt = con.prepareStatement(sql);
        ...
        {
            System.out.println("getidbyuserid Error :" + e.getMessage());
        }
    ...
    public ArrayList<Blogger> getBloggersByStatus(String status) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                blogger.setAddress(rs.getString("address"));
                blogger.setPic(rs.getString("pic"));
                blogger.setApprovedStatus(rs.getString("approvedStatus"));
            list.add(blogger);
            }
